...
int init_engine()
{
        ...
        for(i=0; i<server_count; i++)
        {
                ...
                else if(pid[i] == 0)
                {
                        ...
                        execl(tmp, queuelist[i].svrname, queuelist[i].svcname, NULL);
        ...
        /* Print Process table */
        fprintf(OUTFILE, "%50s\t%s\t%s\n", "SERVER NAME","SERVICE NAME","Processid");
        for(i=0;i<server_count;i++)
        {
                fprintf(OUTFILE, "%50s\t%s\t%d\n", queuelist[i].svrname, queuelist[i].svcname, pid[i]);
        }
